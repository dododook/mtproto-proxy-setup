...
SCRIPT_NAME="mtproxy_menu_v4.sh"
SCRIPT_URL="https://raw.githubusercontent.com/dododook/mtproto-proxy-setup/main/mtproxy_menu.sh"
...
echo -e "3. 退出"
echo -e "4. 更新本脚本"
read -rp "请输入选项 [1-4]: " menu

if [[ $menu == 4 ]]; then
    echo -e "${GREEN}正在从 GitHub 更新脚本...${RESET}"
    if curl -fsSL "$SCRIPT_URL" -o "$SCRIPT_NAME"; then
        chmod +x "$SCRIPT_NAME"
        echo -e "${GREEN}✅ 脚本更新完成，重新运行中...${RESET}"
        exec ./"$SCRIPT_NAME"
    else
        echo -e "${RED}❌ 脚本更新失败，请检查网络或链接是否有效。${RESET}"
    fi
    exit 0
fi
